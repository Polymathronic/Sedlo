/*
Testing Ver2.0
Extracts COP coordinates from pressure mat and sends them via Bluetooth
*/
...
// Temporary string placeholder for message to be received via BT
String incomingMsg;

// Temporary string placeholder for message to be sent via BT
String outgoingMsg;
...
void loop() {
  ...
  // Read input from phone (if needed)
  while (Serial1.available() > 0)  {
    incomingMsg += char(Serial1.read());
    delay(2);
  }
  ...
  // Prepare and send output to phone
  outgoingMsg = String(copX) + "," + String(copY);
  
  char* buf = (char*) malloc(sizeof(char)*outgoingMsg.length()+1);
  outgoingMsg.toCharArray(buf, outgoingMsg.length()+1);

  // Send to phone
  Serial1.write(buf);

  Serial.println(outgoingMsg + ". Freeing the memory");
  free(buf);
}
